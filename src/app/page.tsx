...
// ✅ Moved outside Home — fixes hooks-in-loop violation
function StepWrapper({ step }: { step: any }) {
  const ref = useRef<HTMLDivElement>(null);
  const isActive = useInView(ref, { amount: 0.4 });

  return (
    <div ref={ref} className="w-full transition-all duration-700 ease-in-out">
      <Services {...step} active={isActive} />
    </div>
  );
}
...
\n inMind has WhatsApp automation and AI-powered workflows built right in; order confirmations, attempt follow-ups, delivery notifications, return alerts, and custom triggers; all managed from the same platform where your orders live. No integrations. No extra subscriptions.`,
  ...
  return (
    <>
      <Hero />
      <Counters />
      <section className="w-full relative overflow-hidden bg-[#FAF9F6]">
        <div className="w-full max-w-7xl mx-auto lg:h-auto flex flex-col md:flex-row justify-start items-center gap-6 md:px-0 py-10 md:py-12">
          <div className="w-[88%] lg:w-full mx-auto lg:ms-12 h-auto z-11">
            ...
            <h1 className="font-bold lg:w-full text-[28px] sm:text-[38px] md:text-[40px] leading-tight lg:text-[45px]">
              Your ops team is drowning.
            </h1>
            <h1 className="font-bold text-[28px] md:w-150 lg:w-full sm:text-[38px] md:text-[40px] leading-tight lg:text-[45px] md:mb-10 mb-6.5">
              Not because they're incompetent.
            </h1>
            <p className="font-normal text-[15px] md:w-120 md:text-[20px] lg:w-186 xl:w-210 leading-6 md:leading-7.5 md:pt-0">
      ...
      {/* ✅ Updated: StepWrapper handles active state per step */}
      <div className="relative w-full overflow-hidden min-h-122">
        {steps.map((step) => (
          <StepWrapper key={step.stepNumber} step={step} />
        ))}
      </div>
